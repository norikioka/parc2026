...
- [x] **提出候補3**: `submission_treatment_c_0.85local.zip`（688MB）——episodes_per_task=20版。
      ローカルn=10で85.0%、本番スコア**0.09175**（現行とほぼ同等、誤差範囲）
- [x] **【重要】同一モデルの再提出でノイズ幅を計測** — 2026-08-06
      `submission_0.09397_KEEP.zip`(現行と全く同じファイル)を再提出したところ、本番スコアは
      **0.07203**（1回目0.09397から-23%、絶対差0.022）。**同一モデルでもこの程度ブレる**ことが
      判明し、これまでのtreatment_b(0.0836)・treatment_c(0.09175)の差は、ノイズ幅の中に収まって
      おり統計的に有意とは言えないと判明。「小さな改善では本番のブレを超えられない」という
      重要な教訓（ユーザー指摘で追加検証、2026-08-06）
- [x] **【抜本的な変更】全40タスクでの学習(broad)** — 2026-08-06
      `train_app.py`に`all_tasks`フラグを追加。SPATIAL_TASK_NAMES(黒いボウル10種)限定ではなく、
      `lerobot/libero_plus`の全40タスクを対象に学習(episodes_per_task=5、steps=6000[epoch数を
      controlと揃えるため増加])。汎用性を重視する方向への転換（ユーザー指摘、2026-08-06:
