...
**【2026-08-06確定】ゲートA判定=GREEN。** 初回本番提出でtrack1総合スコア**0.09397**を記録
（参考値0.0633を同一単位で+48%上回る）、クラッシュ・タイムアウト無く正常完走。想定より早く
GREEN条件を満たした。critic Opusレビュー(2026-08-06)により、以下2点を訂正・追加した:

- **【訂正】ローカルModal検証の「62.5%」と本番の「0.09397」は単位が異なり比較不能だった**
  （ローカルは成功率のみ、本番は成功率×衝突ペナルティ×滑らかさの合成スコア）。同一単位で
  比較できるのは「本番スコア vs 参考値0.0633」のみで、そこでは地味だが確かな正の改善が出ている。
  「壊滅的な汎化失敗」という当初の懸念は誤りだった
- **【訂正】「LIBERO-Spatial限定学習が原因」という8/4来の仮説は出典不在と判明**。公式説明会
  資料3点(`docs/PARC2026開発コンペティション_予選.pdf`等)を全文検索したが「L2〜L5」「Object/Goal」
  の記載は無く、むしろTrack1は「同一タスク・同一ドメインへのテクスチャ等の摂動評価」と明記されて
  いた。この仮説を根拠に大規模なタスクスイート拡張・再学習には進まない
- **【新規・未検証】`n_action_steps`50→10への変更が、成功率を上げた一方で滑らかさスコア
  (jerk/SPARC)を悪化させている可能性**。`select_action()`はチャンク境界の補間を行わない単純な
  dequeであり(`src/parc2026/vendor/lerobot/policies/smolvla/modeling_smolvla.py`)、
  n_action_steps=10は600ステップ中の「チャンクの飛び」を12回→60回に5倍増やす。Modal上でGPU
  1〜2時間・提出不要でA/B検証できるため、ステップ4着手前に実施する
- **【要確認・最優先】Omnicampusの提出採用方式（最新 or 最高点）が公式資料に記載が無く未確認**。
  これが分かるまで、現在のスコアを上書きしうる実験的な再提出は控える
